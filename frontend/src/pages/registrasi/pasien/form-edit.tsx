...
import { JaminanT, JenisKunjunganT, RuanganT } from "./types";
...
const FormPasien: FC = () => {
  ...
  const [form, setForm, formFn] = useXState<_Payload, _Response>(
    ...
    {
      method: "POST",
      url: `/registrasi-pelayanan/edit-registrasi-pelayanan/${param.id}`,
    }
  );

  const [registrasiPasien, setRegistrasiPasien] = useState<_Data | null>(null);

  type _Payload = {
    id_ruangan_asal: null | number;
    id_jenis_kunjungan: null | number;
    id_jaminan: null | number;
  };
  ...
  const getRegistrasiPasienById = async () => {
    try {
      const response = await api.get<ResponseT<_Data>>(
        `/registrasi-pelayanan/get-registrasi-pelayanan/${param.id}`
      );

      if (response.data.data) {
        setRegistrasiPasien(response.data.data);
        return response.data.data;
      } else {
        return null;
      }
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const _pasien = await getRegistrasiPasienById();

        if (_pasien) {
          setForm({
            id_ruangan_asal: _pasien.id_ruangan_asal,
            id_jenis_kunjungan: _pasien.id_jenis_kunjungan,
          });
          if (_jaminans) {
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <div className="flex flex-col mb-3">
                <span className="font-medium">{registrasiPasien?.nama}</span>
                <span>{registrasiPasien?.nik}</span>
              </div>
              {registrasiPasien ? (
                <button
                  type="button"
                ...
                <div className="bg-white shadow-lg p-6 rounded-md grid grid-cols-1 gap-4 auto-rows-min">
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      value={registrasiPasien?.tanggal_registrasi ?? ""}
                    />
                  </div>
